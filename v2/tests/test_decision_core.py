...
def test_decision_engine_rejects_excluded_location():
    report = evaluate_job(
        JobFacts(title="Fraud Analyst", company="Example", location="US Remote", description="Fraud and compliance operations", remote=True),
        DecisionContext(target_locations=("Ottawa", "Remote Canada", "Canada"), target_keywords=("fraud", "compliance"), excluded_locations=("United States", "US Remote")),
    )
...
def test_decision_engine_scores_relevant_canadian_role():
    report = evaluate_job(
        JobFacts(
            ...
            description=("Investigate fraud alerts, follow AML and compliance controls, document evidence, support credit-card disputes, "
                         "and communicate with customers in English and French. The analyst works with banking operations and payment-risk teams."),
        ),
        DecisionContext(target_locations=("Ottawa", "Remote Canada", "Canada"), target_keywords=("fraud", "compliance", "bilingual", "aml", "credit"), excluded_locations=("United States", "US Remote"), excluded_titles=("software engineer",)),
    )
    assert report.decision is Decision.SHORTLIST
    assert report.score >= 60
    assert "fraud" in report.matched_keywords
    assert {dimension.name for dimension in report.dimensions} == {"role_relevance", "location_fit", "sector_fit", "language_fit", "seniority_fit", "evidence_quality"}


def test_company_name_does_not_create_role_overlap():
    report = evaluate_job(
        JobFacts(title="Office Administrator", company="Fraud Solutions Inc", location="Ottawa, Ontario, Canada", description="Manage calendars, invoices, records, and office supplies for the local team."),
        DecisionContext(target_locations=("Ottawa",), target_keywords=("fraud",)),
    )
    assert report.decision is Decision.REJECT
...
def test_unicode_blacklist_and_keyword_normalization_is_not_empty():
    description = ("Analyser les dossiers de conformité, évaluer les risques financiers, documenter les contrôles, préparer les rapports de suivi, "
                   "collaborer avec les équipes opérationnelles et signaler les écarts selon les procédures internes et les exigences réglementaires applicables.")
    rejected = evaluate_job(
        JobFacts(title="Analyste conformité", company="Example", location="Montréal, Québec", description=description),
        DecisionContext(target_locations=("Montréal",), target_keywords=("conformité",), blacklisted_companies=("Société interdite",)),
    )
    assert rejected.decision is Decision.SHORTLIST
    blacklisted = evaluate_job(
        JobFacts(title="Analyste conformité", company="Société interdite", location="Montréal, Québec", description=description),
        DecisionContext(target_locations=("Montréal",), target_keywords=("conformité",), blacklisted_companies=("Société interdite",)),
    )
...
def test_sensitive_answer_requires_explicit_provenance():
    vault = AnswerVault([AnswerRecord("work_authorization", "Authorized", AnswerSource.RESUME, confidence=1.0)])
...
def test_sensitive_alias_is_canonicalized():
    vault = AnswerVault([AnswerRecord("WORK_AUTHORIZATION", "Authorized to work in Canada", AnswerSource.USER, confidence=1.0)])
...
def test_empty_required_answer_is_missing():
    vault = AnswerVault([AnswerRecord("work_authorization", "", AnswerSource.USER, confidence=1.0)])
    result = vault.resolve(FieldRequest("work_authorization"))
